...
describe.skip('Unit: BlockParamsHelper', function() {
  it('"blockParams()" returns an object with a property "params" that is an "Array"', function() {
    ...
    expect(result, 'First array element is set').to.have.deep.property(
      'params[0]',
      '0'
    );

    expect(result, 'Second array element is set').to.have.deep.property(
      'params[1]',
      '1'
    );

    expect(result, 'Third array element is set').to.have.deep.property(
      'params[2]',
      '2'
    );
  });
